...
import { 
  History, Coins, Shield, Terminal, Check, Copy, User, Bot, Loader2, Square, Send, Paperclip, X, LogOut, Sparkles 
} from "lucide-react"
...
export default function Home() {
  ...
  const handleSend = async (e?: any, textOverride?: string) => {
    ...
    setMessages(prev => [...prev, uiMsg]); // 先让用户消息上屏
    ...
    setTimeout(async () => {
      try {
        // ✨ 重点修复：这里将包含新消息的完整历史记录发给后端，解决“不回答第二个问题”的 BUG
        const response = await fetch('/api/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
            messages: messages.concat({ role: 'user', content: content }), // 拼接历史
            model 
          }),
          signal: ctrl.signal
        });
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-8 bg-gradient-to-t from-white via-white/90 to-transparent">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="relative shadow-2xl rounded-[32px] overflow-hidden border border-slate-200 bg-white group focus-within:border-blue-200 transition-all text-slate-900">
            {isLoading ? (<Button onClick={()=>abortRef.current?.abort()} className="w-full bg-slate-50 text-slate-500 h-14 rounded-none gap-2 font-black border-none hover:bg-slate-100 transition-colors text-slate-900"><Square size={14} fill="currentColor"/> 停止</Button>) : (
