...
function Thinking({ modelName }: { modelName: string }) {
  ...
  const plan = [
    { title: "一、 需求语义深度解析", steps: ["提取关键词核心意图", "检索历史上下文关联", "构建语义图"] },
    { title: "二、 知识库实时广度检索", steps: ["跨域检索知识节点", "验证数据准确性"] },
    { title: "三、 响应架构多重建模", steps: ["逻辑推理路径模拟", "优化语言表达风格"] },
    { title: "四、 生成结果合规性自检", steps: ["安全性策略匹配", "逻辑闭合终审校验"] }
  ];
  useEffect(() => {
    let m1 = 0; let m2 = -1;
...
export default function Home() {
  ...
  const handleSend = async (e?: any, textOverride?: string) => {
    ...
    setMessages(prev => [...prev, uiMsg]); setInput(""); setImages([]); setFile(null); setIsLoading(true);
    const ctrl = new AbortController(); abortRef.current = ctrl;
    setTimeout(async () => {
      try {
        const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: messages.concat({ role: 'user', content: content }), model }), signal: ctrl.signal });
        const reader = response.body?.getReader();
        const decoder = new TextDecoder();
        let full = ""; setMessages(prev => [...prev, { role: 'assistant', content: "" }]);
        while (true) {
          const { done, value } = await reader?.read()!;
          if (done) break;
          full += decoder.decode(value);
          setMessages(prev => { const last = [...prev]; last[last.length - 1].content = full; return last; });
        }
  ...
  if (!user) return <AuthPage onLogin={(u)=>{ setUser(u); const init = { id: 'init', type: 'topup' as const, amount: '0.10', description: '注册赠送', time: new Date().toLocaleString() }; setTransactions([init]); localStorage.setItem(`tx_${u.id}`, JSON.stringify([init])); }} />;
    ...
    <div className="flex flex-col min-h-screen bg-white text-slate-900 overflow-x-hidden">
        ...
        <div className="flex items-center gap-4">
           ...
           <Dialog open={isProfileOpen} onOpenChange={setIsProfileOpen}>
             ...
             <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none rounded-3xl shadow-2xl bg-white text-slate-900">
               ...
               <div className="p-6 bg-slate-50/50 text-slate-900"><div className="bg-white rounded-2xl p-5 border shadow-sm mb-6"><div className="flex justify-between items-start mb-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-slate-400"><span>可用余额</span><button onClick={()=>{setIsProfileOpen(false); setTimeout(()=>setIsRechargeOpen(true),200)}} className="text-blue-600 font-bold text-slate-900">充值</button></div><div className="text-4xl font-black font-mono text-slate-900">${user.balance}</div></div><div className="space-y-4"><div className="flex items-center gap-2 font-bold text-[10px] text-slate-500 uppercase tracking-widest text-slate-400"><History size={12}/> 记录</div><div className="max-h-[120px] overflow-y-auto space-y-2 pr-1 scrollbar-hide text-[11px] text-slate-900">{transactions.map(t=>(<div key={t.id} className="flex justify-between bg-white p-2.5 rounded-xl border border-slate-100 font-bold text-slate-900"><span>{t.description}</span><span className={t.type==='topup'?'text-green-600':'text-slate-500'}>${t.amount}</span></div>))}</div></div></div>
             </DialogContent>
           </Dialog>
        </div>
      </nav>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-8 bg-gradient-to-t from-white via-white/90 to-transparent">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="relative shadow-2xl rounded-[32px] overflow-hidden border border-slate-200 bg-white group focus-within:border-blue-200 transition-all text-slate-900 text-slate-900">
